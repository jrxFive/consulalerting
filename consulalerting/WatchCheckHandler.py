...
class WatchCheckHandler(Settings):
    ...
    def consulAPILookups(self):
        ...
        try:
            ...
            WatchCheckHandler.logger.debug(
                "HealthCheckTags={tags}".format(
                    tags=self.health_check_tags))
        except KeyError:
            WatchCheckHandler.logger.error(
                "Message=Could not obtain system check tags from\
        ...
        try:
            ...
            WatchCheckHandler.logger.debug(
                "PriorConsulHealth={health}".format(
                    health=self.health_prior))
        except KeyError:
            self.health_prior = []
            WatchCheckHandler.logger.warn(
                "Message=No previous prior catalog health found from\
                 ConsulURI={location}".format(
                    location=WatchCheckHandler.KV_PRIOR_STATE))
    ...
    def filterByBlacklists(self, object_list):
        ...
        try:
            ...
            WatchCheckHandler.logger.debug(
                "FilteredConsulHealthStructList={filtered_list}".format(
    ...
    def createConsulHealthList(self, object_list):
        ...
        try:
            ...
            WatchCheckHandler.logger.debug(
                "ConsulHealthList={listobjects}".format(
                    listobjects=object_list))
            return object_list

        except TypeError:
            WatchCheckHandler.logger.error(
                "Message=createConsulHealthList failed,\
    ...
    def checkForAlertChanges(
        ...
        if not health_prior_object_list:
            ...
            try:
                ...
                if alert_list:
                    WatchCheckHandler.logger.debug(
                        "NoPriorAlertList={alert_list}".format(
                ...
                WatchCheckHandler.logger.error(
                    "Message=Failed to create alert list with\
                     no prior catalog")
                raise
            ...
            try:
                ...
                if alert_hash_set:
                    ...
                    WatchCheckHandler.logger.debug(
                        "PriorAlertList={alert_list}".format(
                            alert_list=alert_list))
                    return alert_list
                else:
    ...
    def Run(self):
        ...
        WatchCheckHandler.logger.info(
            "Message=Creating current and prior health\
        ...
        if alert_list:
            WatchCheckHandler.logger.info(
                "Message=AlertsCreated={numAlerts}".format(
                    numAlerts=len(alert_list)))
