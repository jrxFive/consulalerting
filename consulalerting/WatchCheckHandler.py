...
class WatchCheckHandler(Settings):
    ...
    def consulAPILookups(self):
        """
        Get prior state if any, the current state. Since checks do not have
        a tag attribute, use the ConsulKV alerting/healthchecktags
        to determine who will get notified by what
        """
        try:
            self.health_current = self.consulate_session.health.state("any")

            WatchCheckHandler.logger.debug("CurrentConsulHealth={health}".format(
                health=self.health_current))
        except KeyError:
            WatchCheckHandler.logger.error(
                "Message=Could not obtain current catalog from consul "
                "ConsulURI={uri}".format(uri=self.consulate_session._base_uri))
            raise

        try:
            self.health_check_tags = self.consulate_session.kv[
                WatchCheckHandler.KV_ALERTING_HEALTH_CHECK_TAGS]
        ...
        try:
            self.health_prior = self.consulate_session.kv[
                WatchCheckHandler.KV_PRIOR_STATE]
            WatchCheckHandler.logger.debug(
                "PriorConsulHealth={health}".format(health=self.health_prior))
        except KeyError:
            self.health_prior = []
            WatchCheckHandler.logger.warn(
                "Message=No previous prior catalog health found from "
                "ConsulURI={location}".format(location=WatchCheckHandler.KV_PRIOR_STATE))
    ...
    def alertsBlacklist(self):
        """
        Obtain blacklist options, if KeyError these keys do not exist
        in Consul KV
        """
        try:
            self.node_blacklist = set(self.consulate_session.kv[
                WatchCheckHandler.KV_ALERTING_BLACKLIST_NODES])

            WatchCheckHandler.logger.debug("NodeBlacklist={blacklist}".format(
                blacklist=self.node_blacklist))

        except KeyError:

            WatchCheckHandler.logger.error("Message=Could not obtain node blacklist from "
                                           "ConsulURI={location}".format(
                                               location=WatchCheckHandler.KV_ALERTING_BLACKLIST_NODES))
            raise

        try:
            self.service_blacklist = set(self.consulate_session.kv[
                WatchCheckHandler.KV_ALERTING_BLACKLIST_SERVICES])

            WatchCheckHandler.logger.debug("ServiceBlackList={blacklist}".format(
                blacklist=self.service_blacklist))

        except KeyError:
            WatchCheckHandler.logger.error("Message=Could not obtain service blacklist from "
                                           "ConsulURI={location}".format(
                                               location=WatchCheckHandler.KV_ALERTING_BLACKLIST_SERVICES))
            raise

        try:
            self.check_blacklist = set(self.consulate_session.kv[
                WatchCheckHandler.KV_ALERTING_BLACKLIST_CHECKS])
            WatchCheckHandler.logger.debug("CheckBlackList={blacklist}".format(
                blacklist=self.check_blacklist))
        except KeyError:
            WatchCheckHandler.logger.error("Message=Could not obtain check blacklist from "
                                           "ConsulURI={location}".format(
                                               location=WatchCheckHandler.KV_ALERTING_BLACKLIST_CHECKS))
            raise
    ...
    def filterByServiceBlacklist(self, object_list):
        filtered_services = [
            ...
            if obj.ServiceName not in self.service_blacklist]
    ...
    def filterByCheckBlacklist(self, object_list):
        filtered_checks = [
            obj for obj in object_list
            if obj.ServiceName not in self.check_blacklist]
    ...
    def filterByNodeBlacklist(self, object_list):
        ...
        filtered_nodes = [
            obj for obj in object_list
            if obj.Node not in self.node_blacklist]
    ...
    def filterByBlacklists(self, object_list):
        ...
        try:
            ...
            WatchCheckHandler.logger.debug("FilteredConsulHealthStructList={filtered_list}".format(
    ...
    def createConsulHealthList(self, object_list):
        ...
        try:
            ...
            WatchCheckHandler.logger.debug("ConsulHealthList={listobjects}".format(
                listobjects=object_list))
            return object_list

        except TypeError:
            WatchCheckHandler.logger.error("Message=createConsulHealthList failed, "
        ...
        if not health_prior_object_list:
            ...
            try:
                ...
                current_state_warning = self.getObjectListByState(
                    health_current_object_list,
                    WatchCheckHandler.WARNING_STATE)
                current_state_critical = self.getObjectListByState(
                    health_current_object_list,
                    WatchCheckHandler.CRITICAL_STATE)
                ...
                if alert_list:
                    WatchCheckHandler.logger.debug("NoPriorAlertList={alert_list}".format(
                ...
                WatchCheckHandler.logger.error("Message=Failed to create alert list with "
                                               "no prior catalog")
                raise
            ...
            try:
                health_current_object_set_pass = self.getHashStateSet(
                    health_current_object_list,
                    WatchCheckHandler.PASSING_STATE)
                ...
                health_current_object_set_unknown = self.getHashStateSet(
                    health_current_object_list,
                    WatchCheckHandler.UNKNOWN_STATE)
                ...
                if alert_hash_set:
                    ...
                    WatchCheckHandler.logger.debug("PriorAlertList={alert_list}".format(
                        alert_list=alert_list))
                    return alert_list
    ...
    def Run(self):
        ...
        WatchCheckHandler.logger.info("Message=Creating current and prior health "
                                      "ConsulHealthStruct lists")

        health_current_object_list = self.createConsulHealthList(
            self.health_current)

        health_prior_object_list = self.createConsulHealthList(
            self.health_prior)

        WatchCheckHandler.logger.info(
