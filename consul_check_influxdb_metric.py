...
class InfluxDBQuery(object):
    ...
    def _createSeries(self):
        if self.options.query_custom:
            ...
            self.influx_hostname_series = "{prefix}.{fqdn}.{series}".format(fqdn=fqdn, series=self.options.query_series,
                                                                            prefix=self.options.query_prefix)

        else:
            self.influx_hostname_series = "{prefix}.{hostname}{delimit}{domain}{series}".format(hostname=self.options.query_hostname,
    ...
    def _buildInfluxDBQuery(self):
        ...
        if self.options.query_custom:
            ...
            self.query_string = "select MIN(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                  seconds=self.options.query_min)
        elif self.options.query_max:
            self.query_string = "select MAX(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                  seconds=self.options.query_max)
        elif self.options.query_mean:
            self.query_string = "select MEAN(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                   seconds=self.options.query_mean)
        elif self.options.query_mode:
            self.query_string = "select MODE(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                   seconds=self.options.query_mode)
        elif self.options.query_median:
            self.query_string = "select MEDIAN(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                     seconds=self.options.query_median)
        elif self.options.query_derivative:
            self.query_string = "select DERIVATIVE(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                         seconds=self.options.query_derivative)
        elif self.options.query_sum:
            self.query_string = "select SUM(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                  seconds=self.options.query_sum)
        elif self.options.query_stddev:
            self.query_string = "select STDDEV(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
                                                                                                                     seconds=self.options.query_stddev)
        elif self.options.query_first:
            self.query_string = "select FIRST(value) from {hostname_series} where time > now() - {seconds}s".format(hostname_series=self.influx_hostname_series,
    ...
    def _messageGenerator(self,threshold_value,current_value):
        ...
        message_template = "InfluxDB Series: {series}" \
                           " Query Performed: {query}".format(series=self.influx_hostname_series,
                                                              query=self.query_string)

        message_values = " Current Value: {current}, Threshold Value:{threshold}".format(current=current_value,
    ...
    def _compareQueryToThresholds(self):
        ...
        if self.options.threshold_lessthan == False:

            if self.options.threshold_warning:

                if self.options.threshold_critical <= current_value:
                    self._messageGenerator(self.options.threshold_critical,current_value)
                    sys.exit(InfluxDBQuery.EXIT_CRITICAL)

                if self.options.threshold_warning < current_value and current_value <= self.options.threshold_critical:
                    self._messageGenerator(self.options.threshold_warning,current_value)
                    sys.exit(InfluxDBQuery.EXIT_WARNING)
...
def set_cli_parameters():
    ...
    parser.add_option("--influxdb-ip", action="store", type="string",
    ...
    parser.add_option("--influxdb-pass", action="store", type="string", dest="influxdb_pass",
    ...
    parser.add_option("--query-hostname", action="store", type="string", dest="query_hostname", help="Hostname shown in InfluxDB of the time series to check thresholds: \n \
    name")

    parser.add_option("--query-domain", action="store", type="string", dest="query_domain", help="Domain shown in InfluxDB of the time series to check thresholds \n \
    domain")

    parser.add_option("--query-series", action="store", type="string", dest="query_series", help="Series name shown in InfluxDB, can check using web interface 'list series': \n \
    cpu.total.idle")

    parser.add_option("--query-count-seconds", action="store", dest="query_count", type="int",
                      help="Compare threshold over the X seconds count")

    parser.add_option("--query-min-seconds", action="store", dest="query_min", type="int",
                      help="Compare threshold over the X seconds min")

    parser.add_option("--query-max-seconds", action="store", dest="query_max", type="int",
                      help="Compare threshold over the X seconds max")

    parser.add_option("--query-mean-seconds", action="store", dest="query_mean", type="int",
                      help="Compare threshold over the X seconds mean")

    parser.add_option("--query-mode-seconds", action="store", dest="query_mode", type="int",
                      help="Compare threshold over the X seconds average")

    parser.add_option("--query-median-seconds", action="store", dest="query_median", type="int",
                      help="Compare threshold over the X seconds mode")

    parser.add_option("--query-derivative-seconds", action="store", dest="query_derivative", type="int",
                      help="Compare threshold against the derivative")

    parser.add_option("--query-sum-seconds", action="store", dest="query_sum", type="int",
                      help="Compare threshold against the sum")

    parser.add_option("--query-stddev-seconds", action="store", dest="query_stddev", type="int",
    ...
    parser.add_option("--query-replace-periods-with", action="store", dest="query_replace",
                      help="Usually InfluxDB fqdn will not show up as hostname.domain but instead as hostname_domain, pick delimiter", default="_")

    parser.add_option("--query-time-precision", action="store", dest="query_time_precision",
                      help="InfluxDB timestamp is a microsecond epoch, default is milliseconds", default="ms")

    parser.add_option("--threshold-less-than", action="store_true", dest="threshold_lessthan",
                      help="Instead of looking for greater than critical or warning value, check when less than critical or warning", default=False)

    parser.add_option("--threshold-warning", action="store", type="float", dest="threshold_warning",
                      help="When equal to or exceeding this value but below the critical threshold exit with code 1")

    parser.add_option("--threshold-critical", action="store", type="float",
                      dest="threshold_critical", help="When equal to or exceeding this value exit with code 2")
    ...
    if options.query_full or options.query_custom:
        ...
        if all([options.query_fqdn,options.query_series]):
            ...
            if all([options.query_hostname,options.query_domain,options.query_series]):
                pass
            else:
                parser.error("query option required, eithre just --query-full, or (--query-fqdn and --query-series), or "
