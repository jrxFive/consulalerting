...
class SocketCheck(object):
    ...
    def _verifyRequirements(self):
        ...
        if not (
                self.option.socket_ip and self.option.socket_input and self.option.socket_output):
            sys.stdout.write(
                "Require --ip , --command-input, --command-output")
            sys.exit(1)
    ...
    def _evaluateOutput(self, data):
        if self.option.socket_output in data:
            sys.stdout.write("Output: {result}".format(result=data))
            sys.exit(0)
        else:
            sys.stdout.write(
                "--command-output not found in returned data.\
                Expected Output: {command_output},\
                received_output: {recv_output}".format(
                    command_output=self.option.socket_output,
                    recv_output=data))
            sys.exit(2)
    ...
    def Run(self):
        ...
        try:
            ...
            sys.stdout.write(
                "Socket Error: Error Number: {error_number}, Error String:\
                 {error_string}".format(
            ...
            sys.stdout.write(
                "Socket timeout, timeout: {timeout_value}".format(
                    timeout_value=self.option.socket_timeout))
            sys.exit(1)
