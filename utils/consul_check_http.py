...
class HttpCheck(object):
    ...
    def _verifyRequirements(self):
        ...
        if not (
                self.option.http_ip or self.option.http_address) or (
                self.option.http_ip and self.option.http_address):
    ...
    def _buildRequest(self):
        ...
        if self.option.http_ip:
            ...
            request = "http://{address}".format(
                address=self.option.http_address)

        if self.option.http_port:
            request = "".join(
                [request, ":{port}".format(port=self.option.http_port)])

        if self.option.http_endpoint:
            request = "".join(
                [request, "{endpoint}".format(endpoint=self.option.http_endpoint)])
    ...
    def Run(self):
        ...
        try:
            request_exception_string = "Request String: {request}".format(
                request=request)
            ...
            sys.stdout.write(
                "A Connection error occurred. {request_info}\n".format(
                    request_info=request_exception_string))
            sys.exit(1)
        except requests.exceptions.HTTPError:
            sys.stdout.write(
                "An HTTP error occurred. {request_info}\n".format(
                    request_info=request_exception_string))
            sys.exit(1)
...
def set_cli_parameters():
    ...
    parser.add_option(
        ...
        type="float",
        dest="http_timeout",
        help="Timeout wait period till reporting critical error",
        default=3.0)
